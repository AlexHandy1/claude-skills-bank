...
| [OpenRouter](https://openrouter.ai/rankings#benchmarks) | Unified API across hundreds of models — accessible via the OpenAI SDK client pointed at its base URL, so you can switch or compare providers without code changes. Particularly good for finding the most efficient model for a given task: the [rankings/benchmarks page](https://openrouter.ai/rankings#benchmarks) plots a price-vs-performance Pareto curve for picking the cheapest model that clears your quality bar |
